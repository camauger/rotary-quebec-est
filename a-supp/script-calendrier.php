...
echo "tabActivites = new Array();\n";  // Tableau des noms d'activités
echo "tabActiviteId = new Array();\n"; // Tableau des ID des activités
echo "tabActiviteDate = new Array();\n"; // Tableau des dates de début des activités
echo "tabActiviteDateFin = new Array();\n"; // Tableau des dates de fin des activités
...
// Requête retournant le type des activités
...
// Retourne un tableau des noms des activités
...
echo "<script src='../../_includes/calendar.js'></script>\n";
...
if(isset($_GET["date"]) && $_GET["date"] != "" && !isset($_POST["btnActiviteMois"]))
	$formAction = "calendrier.php?date=".$_GET["date"];
else  // Lorsque le bouton des activités du mois est enfoncé
	$formAction = "calendrier.php";
	
// Selon la langue les mois s'affiche en français ou en anglais
switch($lang)
{
	case "fr":
		$janvier = 'Janvier';
...
?>
<form name="form_calendar" id="formCalendrier"method="post" action="<?=$formAction?>">
...
for($i = $varDate - 6 ; $i < $varDate + 10; $i++)
{	
